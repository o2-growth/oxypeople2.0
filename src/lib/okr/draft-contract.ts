/**
 * Contrato da proposta de OKR gerada pelo copiloto.
 *
 * O modelo não devolve prosa: devolve este schema, validado pela própria API
 * (`output_config.format`) e conferido de novo aqui antes de encostar no
 * formulário. O front nunca faz parsing de texto.
 *
 * ESPELHO: `supabase/functions/_shared/okr-draft-contract.ts` tem a mesma
 * definição porque a edge function roda em Deno e não enxerga `src/`.
 * `draft-contract.parity.test.ts` quebra se as duas divergirem.
 */
