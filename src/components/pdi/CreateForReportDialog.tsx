...
import { useCreatePDIForReport } from "@/hooks/usePDI";
...
export function CreateForReportDialog({ open, onOpenChange, report }: Props) {
  const { profile } = useUser();
  const companyId = profile?.primary_company_id;
  const createPDI = useCreatePDIForReport();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { title: "", description: "", target_date: "", cycle_id: "" },
  });

  useEffect(() => {
    if (open) form.reset({ title: "", description: "", target_date: "", cycle_id: "" });
  }, [open, form]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Criar PDI para {report.full_name ?? "liderado"}</DialogTitle>
        </DialogHeader>

        <div className="flex items-center gap-2 rounded-md border bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
          <Lock className="h-3.5 w-3.5 shrink-0" />
          <span>PDI será criado para <strong>{report.full_name}</strong> com você como gestor responsável.</span>
        </div>
