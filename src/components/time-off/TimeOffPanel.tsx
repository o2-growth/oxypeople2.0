...
import { DaysControlTable } from "@/components/time-off/DaysControlTable";
...
/**
 * Painel de Férias / Ausências.
 *
 * Vive fora da página porque o mesmo painel aparece em dois lugares: a rota
 * /time-off, que já existia, e a aba Férias dentro do RH — que é onde o
 * assunto pertence. Duplicar a tela deixaria as duas divergindo na primeira
 * correção.
 */
export function TimeOffPanel() {
  ...
  // Controle de dias: mesma fonte do alerta, recorte diferente — aqui interessa
  // quanto cada um se afastou e há quanto tempo, não se cruzou um limite.
  const pessoasControle = useMemo(
    () =>
      people
        .filter((p) => p.status === "active")
        .map((p) => ({
          id: p.id,
          nome: p.user?.full_name || p.user?.email || "Sem nome",
          registros: recordsByMember.get(p.id) ?? [],
        })),
    [people, recordsByMember],
  );
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="history">
          <TabsList>
            ...
            <TabsTrigger value="days">Controle de dias</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="days" className="mt-4">
            {list.isLoading ? (
              ...
              <QueryError
                message="Não foi possível carregar os registros de férias."
                onRetry={() => list.refetch()}
              />
            ) : (
              <DaysControlTable pessoas={pessoasControle} />
            )}
          </TabsContent>
        </Tabs>
      </div>
