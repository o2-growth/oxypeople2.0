  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Recebida[]> => {
      ...
      const comPermissao = await Promise.all(
        (data ?? []).map(async (e) => {
          const { data: pode } = await supabase.rpc(
    ...
    <>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                O que outras pessoas responderam a seu respeito
              </CardDescription>
            </div>
            {media != null && (
        ...
        <CardContent className="space-y-2">
          {data.map((r) => {
            const origem = ORIGEM[r.relationship] ?? {
              label: "Avaliação",
              icon: User,
            };
            ...
            return (
              <div
                ...
                // Ver a nota sem ver o que foi escrito é a metade menos útil: o
                // comentário por atitude é o que a pessoa leva para a conversa
                // com o gestor. O formulário já sabe exibir tudo em leitura.
                onClick={() => !bloqueada && setAberta(r.id)}
                onKeyDown={(e) => {
                ...
                <Avatar className="h-9 w-9">
                  <AvatarImage
                    src={
                      bloqueada
                        ? undefined
                        : (r.evaluator?.avatar_url ?? undefined)
                    }
                  />
                  <AvatarFallback className="text-xs">
                    {bloqueada ? (
                      ...
                      (r.evaluator?.full_name ?? "?").slice(0, 2).toUpperCase()
                    )}
                  </AvatarFallback>
                </Avatar>
          ...
          <p className="pt-1 text-xs text-muted-foreground">
            Clique em uma avaliação liberada para ler nota e comentário de cada
            atitude. As notas ficam em sigilo até quem avaliou terminar todas as
            avaliações do ciclo — assim ninguém ajusta o que escreveu depois de
            ver a reação.
          </p>
        </CardContent>
      </Card>

      {/* Mesma tela que quem avaliou usou, em leitura: as 12 atitudes com nota
          e comentário. A RLS decide o que aparece — quem não pode ver não vê
          nem abrindo. */}
      <EvaluationForm
        evaluationId={aberta}
