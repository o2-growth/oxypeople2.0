// Canais de notificação externos do pulse:
 ...
 */
export async function sendPulseEmails(
  targets: EmailTarget[],
  subject: string,
  html: string,
  log: Logger,
  ...
  if (!webhookUrl || !secret) {
    log("info", "pulse-notify:email-skip", { reason: "no N8N webhook/secret" });
    return 0;
  }
  ...
  const results = await mapWithConcurrency(targets, 8, async (t) => {
    try {
      ...
      if (!res.ok) {
        log("warn", "pulse-notify:email-failed", { status: res.status, email: t.email });
        return false;
      }
      return true;
    } catch (err) {
      log("warn", "pulse-notify:email-exception", { msg: (err as Error).message, email: t.email });
      return false;
    }
  });
 ...
 */
export async function sendPulseSlackDMs(
  targets: EmailTarget[],
  text: string,
  log: Logger,
): Promise<number> {
  ...
  if (!botToken) {
    log("info", "pulse-notify:slack-dm-skip", { reason: "no SLACK_BOT_TOKEN" });
    return 0;
  }
  if (targets.length === 0) return 0;
  ...
  const results = await mapWithConcurrency(targets, 5, async (t) => {
    try {
      ...
      if (!lookupData.ok) {
        log("info", "pulse-notify:slack-no-user", { email: t.email, error: lookupData.error });
        return false;
      }
      ...
      if (!postData.ok) {
        log("warn", "pulse-notify:slack-dm-failed", { email: t.email, error: postData.error });
        return false;
      }
      return true;
    } catch (err) {
      log("warn", "pulse-notify:slack-dm-exception", { msg: (err as Error).message, email: t.email });
      return false;
    }
  });
  return results.filter(Boolean).length;
 ...
 */
export async function postPulseSlackChannel(
  channel: string | null,
  text: string,
  log: Logger,
): Promise<boolean> {
  const botToken = Deno.env.get("SLACK_BOT_TOKEN");
  if (!botToken) {
    log("info", "pulse-notify:slack-channel-skip", { reason: "no SLACK_BOT_TOKEN" });
    return false;
  }
  if (!channel) {
    log("info", "pulse-notify:slack-channel-skip", { reason: "no channel configured" });
    return false;
  }
  try {
    ...
    if (!data.ok) {
      log("warn", "pulse-notify:slack-channel-failed", { error: data.error });
      return false;
    }
    return true;
  } catch (err) {
    log("warn", "pulse-notify:slack-channel-exception", { msg: (err as Error).message });
    return false;
  }
}
